...
namespace YasuoBuddy
{
    class StateManager
    {
        ...
        public static void Harass()
        {
            ...
            if (Yasuo.HarassMenu["harass.E"].Cast<CheckBox>().CurrentValue && (target.Distance(pos) > 500) && unit != null && TargetSelector.SelectedTarget == target && (!unit.GetDashPos().IsUnderTower())&& unit.Distance(Player.Instance) > Player.Instance.GetAutoAttackRange(target))
            {
                SpellManager.E.Cast(unit);
            }
            if (Player.Instance.HasWhirlwind()) return;
